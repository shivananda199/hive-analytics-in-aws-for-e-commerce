-- enter hive docker container: 
-- > docker exec -i -t ra_hive-server bash
-- then enter hive prompt by typing: hive
-- run below queries
...
load data inpath '/input/v_salesorderheader/part-m-00001' into table sales_order_header;
...
select count(1) from customer_test;
select count(1) from sales_order_header;
select count(1) from sales_order_details;
select count(1) from store_details;
select count(1) from creditcard;
